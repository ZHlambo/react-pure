...
export const getAuthorization = (reducer) => {
  ...
  // return 'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4gRG9lIiwiYWRtaW4iOnRydWV9.TJVA95OrM7E2cBab30RMHrHDcEfxjoYZgeFONFh7HgQ';
  // return {headers:{'Authorization':'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4gRG9lIiwiYWRtaW4iOnRydWV9.TJVA95OrM7E2cBab30RMHrHDcEfxjoYZgeFONFh7HgQ'}}
  // return {headers:{'Authorization':'Bearer ' + _value}}
}
...
export const shadowDiffer = (first, second, result) => {
  ...
  //根据30812转换具体时间
export const formatterTime = (standtime, formattertime) => {
  if (!(standtime && formattertime)) {
    return '无';
  }
  if (formattertime.length != 5) {
    return 'null';
  }
  var time1 = formattertime.substr(0, 1);
  var time2 = formattertime.substr(1, 4);
  var datetemp = new Date(standtime);
  var date = datetemp.getTime();
  var returndate = new Date(date + Number(time1 - 1) * 86400000).toLocaleDateString();
  // if (time2 == '0812') {
  //   returndate = returndate + ' 上午';
  // } else if (time2 == '0818') {
  //   returndate = returndate + ' 一天';
  // } else {
  //   returndate = returndate + ' 下午';
  // }
  return returndate;
}
